...
| `verify-992a.sh` | the 992 XDP hook verifier, eleven steps, PASS/FAIL tally. **Stale as of 999**: it tests the generic hook 999 replaces with a native one, so its attach steps still run but what they verify is no longer the path the driver takes. Needs a pass before it is trusted again |
...
# 996: the threaded toggle must be inert, not fatal. This took the WAN down
