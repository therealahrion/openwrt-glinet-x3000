...
| firewall4 flowtable device list (2026-09-12) | `900-flowtable-fall-back-to-l3-device.patch` against the firewall4 package. Stock fw4 builds a flowtable's device list from each zone's `related_physdevs`, which is fed from a network's physdev - and an L3-only interface has none, so `wwan0` never enters the flowtable and the row above shortcuts nothing on the WAN side. The patch falls back to the L3 device. Without it `bpf_xdp_flow_lookup()` has no entry to find, so this is a precondition for the whole flowtable-driven XDP line of work, not a tidy-up. Rationale in `xdp-methods-tested.md` 10.3 | `package/network/config/firewall4/patches/900-flowtable-fall-back-to-l3-device.patch` |
