...
| `verify-xdp.sh` | the XDP verifier for the whole 991-999 series, eleven steps, PASS/FAIL tally. Renamed from `verify-992a.sh` on 2026-09-15, because it stopped being about one patch. **Its 992 steps are stale as of 999**: they exercise the generic hook 999 replaces, so the attach still runs but what it verifies is no longer the path the driver takes. Extend this rather than writing another one-off sampler |
